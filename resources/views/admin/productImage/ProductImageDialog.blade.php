...
{{-- <div class="modal fade" id="addressDialog" tabindex="-1" role="dialog" aria-labelledby="addressLabel"> --}}
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      {!! Form::open(array('url' => array('admin/addProductImage/'.$result['product_id']), 'name'=>'addProductImageFrom', 'id'=>'addProductImageFrom', 'method'=>'post', 'class' => 'form-horizontal', 'enctype'=>'multipart/form-data')) !!}
      @elseif ($result['operation'] == 'view_edit')
      {!! Form::open(array('url' => array('admin/updateProductImage/'.$result['product_id']), 'name'=>'editProductImageFrom', 'id'=>'editProductImageFrom', 'method'=>'post', 'class' => 'form-horizontal', 'enctype'=>'multipart/form-data')) !!}
      @endif
    {!! Form::text('product_id',  $result['product_id'], array('class'=>'form-control', 'id'=>'product_id')) !!}
      ...
      <div class="modal-footer">
        ...
        @if ($result['operation'] == 'listing' || $result['operation'] == 'view_add' )
          <button type="submit" class="btn btn-primary" id="AddProductImage">{{ trans('labels.AddProductImage') }}</button>
        @elseif ($result['operation'] == 'view_edit')
          <button type="submit" class="btn btn-primary" id="EditProductImage">{{ trans('labels.EditProductImage') }}</button>
        @endif
      </div>
      {!! Form::close() !!}
    </div>
