...
class ProductAttributeService extends BaseApiService{
    ...
    //     Log::info('[ProductAttribute] -- getListing : ' .json_encode($product_image_array));
    //     return $product_image_array;
    // }

    // function getProductAttribute($product_image_id){
    ...
    //     Log::info('[ProductAttribute] -- getProductAttribute : ' .json_encode($product_image));
    ...
    function redirect_view($result,$title){
        ...
        switch($result['operation']){
            case 'listing':
                $result["product_attributes"] = $this->getListing($result['product_id']);
                Log::info('[listing] --  : ' . \json_encode($result));
                return view("admin.ProductAttribute.listingProductAttribute", $title)->with('result', $result);
            break;
            case 'view_add':
                // Log::info('[view_add] --  : ' . \json_encode($result));
                return view("admin.ProductAttribute.ProductAttributeDialog", $title)->with('result', $result);
            break;
            case 'view_edit':
                // Log::info('[view_edit] --  : ');
                $result['product_image'] = $this->getProductAttribute($result['product_image_id']);
                Log::info('[view_edit] --  : ' . \json_encode($result));
                return view("admin.ProductAttribute.ProductAttributeDialog", $title)->with('result', $result);
            break;
            case 'add':
                //  Log::info('[add] --  : '. \json_encode($result["product_attribute"]));
                try{
                    DB::beginTransaction();
